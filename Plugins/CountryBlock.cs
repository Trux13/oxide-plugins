...
{
    [Info("CountryBlock", "Wulf/lukespragg", "1.2.1", ResourceId = 1920)]
    ...
    class CountryBlock : CovalencePlugin
    {
        ...
        {
            lang.RegisterMessages(new Dictionary<string, string> { ["PlayerRejected"] = "This server doesn't allow players from {country}" }, this);
        }
        ...
        {
            var ip = IpAddress(player.ConnectedPlayer.Address);
        ...
        {
            if (banInstantly)
                player.Ban(Lang("PlayerRejected", player.Id).Replace("{country}", country), TimeSpan.Zero);
            else
                player.ConnectedPlayer.Kick(Lang("PlayerRejected", player.Id).Replace("{country}", country));
        }
